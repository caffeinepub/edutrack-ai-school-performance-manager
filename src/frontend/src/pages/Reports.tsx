...
export default function Reports() {
  ...
  const { data: marks = [], isLoading: marksLoading } =
    useMarksByStudent(studentIdBigInt);
  const { data: feedbacks = [], isLoading: feedbackLoading } =
    useFeedbackByStudent(studentIdBigInt);
  const { data: analysis, isLoading: analysisLoading } =
    useStudentAnalysis(studentIdBigInt);
        ...
        <div className="print-area space-y-5" id="report-content">
          ...
          <div className="print-only text-center mb-6">
            <h1 className="text-2xl font-bold">
              EduTrack AI — Student Performance Report
            </h1>
              ...
              <Card className="shadow-card">
                ...
                <CardContent className="p-0">
                  {marks.length === 0 ? (
                    ...
                    EXAM_TYPES.filter((et) => groupedMarks[et]).map(
                      (examType) => (
                        <div
                          key={examType}
                                  ...
                                  <span
                                    ...
                                    style={{
                                      color: (() => {
                                        ...
                                        const total = examMarks.reduce(
                                          (s, m) => s + m.marks,
                                          0,
                                        );
                                        const max = examMarks.reduce(
                                          (s, m) => s + m.maxMarks,
                                          0,
                                        );
                                        const pct =
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div>
                        ...
                        <p
                          ...
                          style={{
                            color: getPercentageColor(analysis.overallAverage),
                          }}
                        >
                      ...
                      <div>
                        ...
                        <div className="space-y-1.5">
                          <div className="flex items-center gap-2">
                            <div
                              className={`w-2 h-2 rounded-full ${analysis.isHighRisk ? "bg-red-500" : "bg-green-500"}`}
                            />
                            <span className="text-sm">
                              High Risk: {analysis.isHighRisk ? "Yes" : "No"}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            <div
                              className={`w-2 h-2 rounded-full ${analysis.isWeak ? "bg-orange-400" : "bg-green-500"}`}
                            />
                            <span className="text-sm">
                              Weak Student: {analysis.isWeak ? "Yes" : "No"}
                            </span>
                          </div>
                        </div>
                      </div>
                ...
                <CardContent>
                  {improvementPlan ? (
                    ...
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p className="text-sm">
                        Click "Generate Plan" to create an AI improvement plan
                      </p>
                    </div>
                  )}
                </CardContent>
          ...
          <CardContent className="py-16 flex flex-col items-center gap-4 text-muted-foreground">
            ...
            <div className="text-center">
              ...
              <p className="text-sm mt-1">
                Choose a student to view their comprehensive performance report.
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div
              ...
              style={{ background: "oklch(0.94 0.07 145 / 0.2)" }}
            >
            ...
            <div
              ...
              style={{ borderColor: "oklch(0.475 0.175 255 / 0.3)" }}
            >
              <p className="text-xs text-muted-foreground">
                <strong>Integration Note:</strong> To enable WhatsApp Business
                API integration, connect your WhatsApp Business Account
                credentials in the settings. The above message will be sent
                directly to{" "}
                {selectedStudent?.parentPhone || "the parent's phone number"}.
              </p>
            </div>
          </div>
          <DialogFooter>
