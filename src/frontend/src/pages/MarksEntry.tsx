...
import { ClipboardList, FileText, Loader2, Plus, Upload } from "lucide-react";
import { useRef, useState } from "react";
import { toast } from "sonner";
import type { Mark } from "../backend.d";
import { useAuth } from "../context/AuthContext";
import { useActor } from "../hooks/useActor";
import { useAddMark, useAllMarks, useAllStudents } from "../hooks/useQueries";
...
export default function MarksEntry() {
  ...
  const handleCsvUpload = async () => {
    ...
    try {
      ...
      const dataLines = lines[0].toLowerCase().includes("studentid")
        ? lines.slice(1)
        : lines;
      let successCount = 0;
      let errorCount = 0;
      ...
      for (const line of dataLines) {
        ...
        const marksVal = Number.parseFloat(marksStr);
        const maxMarksVal = Number.parseFloat(maxMarksStr);
        if (
          !studentId ||
          !subject ||
          !examType ||
          Number.isNaN(marksVal) ||
          Number.isNaN(maxMarksVal)
        ) {
          errorCount++;
          continue;
        }
      ...
      if (successCount > 0) {
        toast.success(
          `${successCount} mark${successCount > 1 ? "s" : ""} uploaded successfully`,
        );
      }
      if (errorCount > 0) {
        toast.error(
  ...
  const _marksByStudent = marks.reduce<Record<string, Mark[]>>((acc, mark) => {
        ...
        <TabsContent value="manual" className="mt-4">
          <Card className="shadow-card max-w-2xl">
            ...
            <CardContent className="space-y-4">
              ...
              {form.marks &&
                form.maxMarks &&
                !Number.isNaN(Number.parseFloat(form.marks)) &&
                !Number.isNaN(Number.parseFloat(form.maxMarks)) &&
                Number.parseFloat(form.maxMarks) > 0 && (
                  <div
                    ...
                    <span
                      ...
                      style={{
                        color: getPercentageColor(
                          Math.round(
                            (Number.parseFloat(form.marks) /
                              Number.parseFloat(form.maxMarks)) *
                              100,
                          ),
                        ),
                      }}
            ...
            <CardContent className="space-y-4">
              <div
                ...
                style={{
                  ...
                  background: "oklch(0.475 0.175 255 / 0.04)",
                }}
              >
                ...
                <p className="text-xs text-muted-foreground mb-2">
                  The CSV file should have the following columns (header row is
                  optional):
                </p>
                <div className="font-mono text-xs bg-card border border-border rounded-lg p-3">
                  <p className="text-muted-foreground mb-1">
                    studentId,subject,examType,marks,maxMarks
                  </p>
                      ...
                      return (
                        <TableRow key={mark.id.toString()}>
                          ...
                          <TableCell className="text-right">
                            <span
                              className="font-bold text-sm"
                              style={{ color: getPercentageColor(pct) }}
                            >
                              {pct}%
                            </span>
                          </TableCell>
